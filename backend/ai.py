...
def redact_emails(text: str) -> str:
    """Replaces all email addresses in a string with [REDACTED_EMAIL]."""
    if not text:
        return ""
    # Enhanced regex for email matching
...
COMPANY_ALIASES = {
    ...
    'arla': 'Arla Foods',
    'crown': 'Danish Crown',
    'radiometer': 'Radiometer Medical',
    'unibrew': 'Royal Unibrew',
    'ptc': 'Kepware / PTC',
    'sigmatex': 'Sigmatex UK'
}
...
RULE 2 — TECHNICAL PROBLEM AUDIT (MANDATORY 2-3 SENTENCES):
  - problem_summary MUST be at least 2-3 detailed, technical sentences.
  - WRONG: "Customer has a sensor issue."
  - RIGHT: "Factbird DUO device reporting 'Intermittent Offline' status on Line 4 since firmware v2.1 update. The issue is causing data gaps in the OEE dashboard for the Danish site, impacting shift reporting KPIs."

RULE 3 — STRATEGIC FIX ROADMAP (MANDATORY 2-3 SENTENCES):
  - solution_summary MUST be at least 2-3 detailed, actionable sentences describing exactly what was done or found.
  - WRONG: "Support fixed the issue."
  - RIGHT: "Reconfigured Kepware MQTT tags, updated gateway address to 10.0.x.x, and verified signal flow in Operator View. Support also whitelisted the required IP ranges in the factory firewall to prevent future connectivity drops."
  - Always include specific hardware IDs, error codes, and next-step best practices.
...
RULE 5 — CONFIDENCE SCORING (RESOLUTION-DRIVEN):
  - 95% (CONFIRMED FIX): If the customer says "online now", "working", "everything's okay", or "you may close this". This counts as a 100% confirmed resolution.
  - 90%–100%: Technical resolution confirmed (by support or customer) AND clear customer identity (domain or text).
  - 85%: HIGH CONFIDENCE. Use this if the ticket contains a **Definite Technical Solution** (e.g., config change, firmware update, hardware swap) that is **Confirmed Successfully**.
  - 85% (INTERNAL CONFIRMED): Use this for Infrastructure/Internal tickets (SSO, S3, etc.) where the technical work is **Done and Verified**, even if no external customer name is present.
  - 65% (UNSOLVED/BLOCKED): Use this for tickets where the technical issue is **Unsolved**, **Blocked** (e.g., IT security), or **No clear fix** is mentioned.

RULE 6 — COMPANY NAME (PRECEDENCE RULE):
  - Factbird is the provider, NOT the customer.
  - If the domain is "factbird.com", "superuser.com", or "internal", you MUST search the entire email body/signature for the CUSTOMER name.
  - Look for patterns like "Regards, [Name] ([Company])" or "Hi Support, this is [Company]".
  - If you see "Berry Global", "Novo Nordisk", "Arla", etc. in the text, THAT IS THE COMPANY_NAME regardless of the email address.
  - ONLY use "Factbird / Internal" if no other company name exists in the entire conversation.
...
RULE 9 — PRIVACY & PII PROTECTION (CRITICAL):
  - NEVER include an email address in any output field (problem_summary, solution_summary, reasoning).
  - If you need to refer to a sender, use their first name or role (e.g., "The customer", "Maintenance Engineer", "Christian").
  - Failure to redact emails in these fields constitutes a security violation.

RULE 10 — CONVERSATION PARSING (MANDATORY):
  - The text often contains multiple messages separated by " --- " or "From:".
  - You MUST scan the ENTIRE block. The solution is often in the VERY LAST message.
  - If the final message contains "det ser super ud", "tak", "it works", "solved", or "close the ticket", you MUST award the **95% Confirmed Fix** score.
  - Failure to see the last message is a CRITICAL ERROR. Search for "super" or "tak" at the end.

RULE 11 — FINAL LINE PRIORITY (STATUS UPDATE):
  - The final 2-3 sentences of the text represent the current status.
  - If they contain phrases like "has been replaced", "fixed now", "restored", or "done", this OVERRIDES any earlier requests for meetings or help.
  - In this case, categorize as "Technical Fix" and award **85%-95% Confidence**.

RULE 12 — FUTURE vs. COMPLETED ACTIONS (CRITICAL):
  - If the technical fix or action is described in the **FUTURE** or is **CONDITIONAL** (e.g., "I will do this tomorrow", "When you mount them", "Let me know when you are ready"), it is NOT a completed resolution.
  - In these cases, you MUST cap the confidence score at **65% (UNSOLVED/BLOCKED)**.
  - Only award 85-95% if the work is ALREADY DONE or the solution is ALREADY APPLIED.
...
def extract_text(obj: Any) -> str:
    ...
    if isinstance(obj, dict):
        # 1. Specialized merger for 'output' structure
        if 'output' in obj and isinstance(obj['output'], dict):
            output_dict = obj['output']
            all_parts: List[str] = []
            # Gather all string values, prioritizing order but taking everything
            for k in sorted(output_dict.keys(), key=lambda x: str(x)):
                v = output_dict[k] # pyre-ignore[29]
                if isinstance(v, str) and v.strip():
                    all_parts.append(v.strip())
                elif isinstance(v, dict):
                    all_parts.append(extract_text(v))
            if all_parts:
                return "\n\n -- [MESSAGE SPLIT] -- \n\n".join(all_parts)
            
        # 2. General Dictionary handling
        for key in ['text', 'Email_Text', 'description', 'Description', 'body']:
            val = obj.get(key)
            if val and isinstance(val, str) and len(val) > 20:
                return val
        
        # 3. Handle 'item_' sequence (Individual messages)
        item_keys = sorted([k for k in obj.keys() if k.lower().startswith('item_')])
        if item_keys:
            all_msg_parts: List[str] = []
            for idx, k in enumerate(item_keys):
                role_label = "Customer: " if idx % 2 == 0 else "Support: "
                content_val = obj.get(k, "")
                if isinstance(content_val, dict):
                    content_val = extract_text(content_val)
                all_msg_parts.append(f"{role_label}{content_val}")
            return "\n\n".join(all_msg_parts)
            
        # 4. Fallback: return the first string value found
        for v in obj.values():
            if isinstance(v, str) and v.strip():
                return v.strip()
...
def process_ticket(problem, solution, email, mode='deep'):
    ...
    cache_key = f"{combined_text}_{email}_{analysis_mode}_v15"
        ...
        try:
            ...
            if response and response.text:
                ...
                if res:
                    ...
                    print('[DEBUG] Gemini returned text but JSON parsing failed ❌')
        except Exception as e:
            print(f'[DEBUG] Gemini API failed with error: {e}')
    ...
    if not parsed:
        print('[ERROR] ALL LLM calls failed! Heuristic fallback is currently DISABLED.')
        # parsed = local_heuristic_fallback(combined_text, cleaned_problem, cleaned_solution, email)
        return {"error": "AI analysis failed. Heuristic fallback is disabled for testing."}
    
    # --- FINAL POST-PROCESSING (STRICT TAXONOMY ENFORCEMENT & PRIVACY) ---
    final_result = parsed if isinstance(parsed, dict) else {}
    
    # Redact emails from textual summaries for PII protection
    for field in ["problem_summary", "solution_summary", "reasoning"]:
        if field in final_result:
            final_result[field] = redact_emails(str(final_result[field]))
...
# def local_heuristic_fallback(combined_text: str, cleaned_problem: str, cleaned_solution: str, email: str) -> Dict[str, Any]:
...
#     final_problem: str = ""
#     if problem_sentence:
#         final_problem = str(problem_sentence)
#     elif sentences and len(sentences) > 0:
#         final_problem = str(sentences[0])
#     else:
#         final_problem = cast(Any, combined_text)[:60]
